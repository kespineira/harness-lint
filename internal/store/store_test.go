...
func TestOpenMigratesAndReopensPersistedDatabase(t *testing.T) {
	...
	if status != (SchemaStatus{Current: 7, Latest: 7}) {
		t.Fatalf("schema status = %#v, want current/latest 7", status)
	}
...
func TestOpenMigrationSecondRunIsNoOpForPersistedData(t *testing.T) {
	...
	if status != (SchemaStatus{Current: 7, Latest: 7}) {
		t.Fatalf("schema status after second run = %#v, want current/latest 7", status)
	}
...
func TestOpenUpgradesRepresentativeV4DatabasePreservingUsageHistory(t *testing.T) {
	...
	if status != (SchemaStatus{Current: 7, Latest: 7}) {
		t.Fatalf("upgraded schema status = %#v, want current/latest 7", status)
	}
...
func checkSchema(t *testing.T, s *Store) {
	...
	want := []string{"capabilities", "capability_presence_epochs", "capture_delivery_health", "capture_epochs", "current_inventory", "inventory_scans", "schema_meta", "usage_event_evidence", "usage_events"}
